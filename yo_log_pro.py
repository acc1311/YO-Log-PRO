...
- FIXED: All popup dialogs center on parent window
- FIXED: 'dict' object has no attribute 'insert'
"""
...
# ─── Exchange option keys for Cabrillo export dialog ─────────────────────────
EXCH_SENT_OPTIONS = {
    "ro": {
        "county": "Județ ({jud})",
        "grid": "Locator ({loc})",
...
EXCH_RCVD_OPTIONS = {
    "ro": {
        "log": "Din log (notă/serial)",
        "none": "Nimic (--)",
    },
    "en": {
        "log": "From log (note/serial)",
        "none": "None (--)",
    }
}
...
T = {
    "ro": {
        ...
        "cab2_config": "Configurare Cabrillo 2.0",
        "cab2_export": "📤 Exportă",
    },
    "en": {
        ...
        "cab2_config": "Cabrillo 2.0 Configuration",
        "cab2_export": "📤 Export",
    }
}
...
class Score:
    ...
    def qso(q, rules, cfg=None):
        ...
        if sm == "per_qso":
            ...
            sp2 = rules.get("special_scoring", {})
            return int(sp2.get(call, rules.get("points_per_qso", 1)))
        elif sm == "distance":
            n = q.get("n", "").strip()
            ml = (cfg or {}).get("loc", "")
    ...
    def worked_other(data, call, band, mode):
        ...
        for q in data:
            if q.get("c", "").upper() == cu:
                if q.get("b") != band or q.get("m") != mode:
                    return True
        return False
    ...
    def validate(data, rules, cfg=None):
        ...
        if mq > 0 and len(data) < mq:
            msgs.append(f"⚠ Min {mq} QSO, aveți/you have {len(data)}")
        seen, dc = set(), 0
        for q in data:
            ...
            seen.add(k)
        if dc:
            msgs.append(f"⚠ {dc} duplicate(s)")
        req = rules.get("required_stations", [])
        if req:
            calls_in_log = {q.get("c", "").upper() for q in data}
            missing = [r for r in req if r.upper() not in calls_in_log]
            if missing:
                msgs.append(f"⚠ Stații lipsă / Missing: {', '.join(missing)}")
        ab = rules.get("allowed_bands", [])
        am = rules.get("allowed_modes", [])
        if ab:
            bad_b = sum(1 for q in data if q.get("b") not in ab)
            if bad_b:
                msgs.append(f"⚠ Benzi interzise / Forbidden bands: {bad_b} QSO")
        if am:
            bad_m = sum(1 for q in data if q.get("m") not in am)
            if bad_m:
                msgs.append(f"⚠ Moduri interzise / Forbidden modes: {bad_m} QSO")
        if msgs:
            return False, "\n".join(msgs), 0
...
class Importer:
    ...
    def parse_csv(text):
        ...
        try:
            for row in csv.DictReader(io.StringIO(text)):
                call = (row.get("Call") or row.get("CALL") or row.get("call")
                        or row.get("Callsign") or "").upper().strip()
                if not call:
                    continue
                q = {"c": call,
                     "b": row.get("Band") or row.get("BAND") or "40m",
                     "m": row.get("Mode") or row.get("MODE") or "SSB",
                     "s": row.get("RST_Sent") or row.get("RST_S") or "59",
                     "r": row.get("RST_Rcvd") or row.get("RST_R") or "59",
                     "d": row.get("Date") or row.get("DATE") or datetime.datetime.utcnow().strftime("%Y-%m-%d"),
                     "t": row.get("Time") or row.get("TIME") or "00:00",
                     "f": row.get("Freq") or row.get("FREQ") or "",
                     "n": row.get("Note") or row.get("NOTE") or row.get("Comment") or "",
                     "ss": row.get("Nr_S") or row.get("SS") or "",
                     "sr": row.get("Nr_R") or row.get("SR") or ""}
                qsos.append(q)
        except Exception:
            pass
        return qsos
    ...
    def _parse_cab2_qso(parts):
        try:
            ...
            if len(tk_list) < 8:
                return None
            freq, mode_cab, date_str, time_str = tk_list[0], tk_list[1].upper(), tk_list[2], tk_list[3]
            call = tk_list[7] if len(tk_list) > 7 else ""
            if not call or call == "--":
                return None
            rst_r = tk_list[8] if len(tk_list) > 8 else "59"
            exch_r = tk_list[9] if len(tk_list) > 9 else ""
            mode = CAB2_MODE_REV.get(mode_cab, "SSB")
            d = date_str if "-" in date_str else f"{date_str[:4]}-{date_str[4:6]}-{date_str[6:8]}" if len(date_str) == 8 else date_str
            t = f"{time_str[:2]}:{time_str[2:4]}" if len(time_str) >= 4 else time_str
            return {"c": call.upper(), "b": freq2band(freq) or "40m", "m": mode,
                    "s": tk_list[5] if len(tk_list) > 5 else "59", "r": rst_r,
                    "d": d, "t": t, "f": freq,
                    "n": exch_r if exch_r != "--" else "",
                    "ss": tk_list[6] if len(tk_list) > 6 and tk_list[6] != "--" else "",
    ...
    def _parse_cab3_qso(parts):
        try:
            ...
            if len(tk_list) < 7:
                return None
            freq, mode, date_str, time_str = tk_list[0], tk_list[1].upper(), tk_list[2], tk_list[3]
            call = tk_list[7] if len(tk_list) > 7 else ""
            if not call:
                return None
            d = f"{date_str[:4]}-{date_str[4:6]}-{date_str[6:8]}" if len(date_str) == 8 and "-" not in date_str else date_str
            t = f"{time_str[:2]}:{time_str[2:4]}" if len(time_str) >= 4 else time_str
            return {"c": call.upper(), "b": freq2band(freq) or "40m", "m": mode,
                    "s": tk_list[5] if len(tk_list) > 5 else "59",
                    "r": tk_list[8] if len(tk_list) > 8 else "59",
                    "d": d, "t": t, "f": freq,
                    "n": tk_list[9] if len(tk_list) > 9 else "",
                    "ss": tk_list[6] if len(tk_list) > 6 else "",
                    "sr": tk_list[9] if len(tk_list) > 9 else ""}
        except Exception:
            return None
...
class ContestEditor(tk.Toplevel):
    def __init__(self, parent, cid=None, cdata=None, all_c=None):
        ...
        self.d = copy.deepcopy(cdata) if cdata else {
            "name_ro": "", "name_en": "", "contest_type": "Simplu", "cabrillo_name": "",
            "categories": ["Individual"], "scoring_mode": "none", "points_per_qso": 1,
            "min_qso": 0, "allowed_bands": list(BANDS_ALL), "allowed_modes": list(MODES_ALL),
            "required_stations": [], "special_scoring": {}, "use_serial": False,
            "use_county": False, "county_list": [], "multiplier_type": "none",
    ...
    def _build(self):
        ...
        self._inner = tk.Frame(canvas, bg=TH["bg"], padx=15, pady=10)
        ...
        ttk.Combobox(f, textvariable=self._efv, values=EXCHANGE_FORMATS, state="readonly", width=18).grid(row=r, column=1, sticky="w", pady=3)
        r += 1
        ...
        tk.Checkbutton(f, text=L.t("use_serial"), variable=self._serv, bg=TH["bg"], fg=TH["fg"], selectcolor=TH["entry_bg"], activebackground=TH["bg"]).grid(row=r, column=0, columnspan=2, sticky="w", pady=3)
        r += 1
        self._couv = tk.BooleanVar(value=self.d.get("use_county", False))
        tk.Checkbutton(f, text=L.t("use_county"), variable=self._couv, bg=TH["bg"], fg=TH["fg"], selectcolor=TH["entry_bg"], activebackground=TH["bg"]).grid(row=r, column=0, columnspan=2, sticky="w", pady=3)
        r += 1
        tk.Label(f, text=L.t("cats"), **lo).grid(row=r, column=0, sticky="nw", pady=3)
        self._cats_t = tk.Text(f, width=38, height=4, **eo)
        self._cats_t.insert("1.0", "\n".join(self.d.get("categories", [])))
        self._cats_t.grid(row=r, column=1, sticky="w", pady=3)
        r += 1
        tk.Label(f, text=L.t("a_bands"), **lo).grid(row=r, column=0, sticky="nw", pady=3)
        bf = tk.Frame(f, bg=TH["bg"])
        bf.grid(row=r, column=1, sticky="w", pady=3)
        ab_set = set(self.d.get("allowed_bands", BANDS_ALL))
        self._band_vars = {}
        ...
        mf = tk.Frame(f, bg=TH["bg"])
        mf.grid(row=r, column=1, sticky="w", pady=3)
        am_set = set(self.d.get("allowed_modes", MODES_ALL))
        self._mode_vars = {}
        ...
        self._req_t = tk.Text(f, width=38, height=3, **eo)
        self._req_t.insert("1.0", "\n".join(self.d.get("required_stations", [])))
        self._req_t.grid(row=r, column=1, sticky="w", pady=3)
        r += 1
        tk.Label(f, text=L.t("sp_sc"), **lo).grid(row=r, column=0, sticky="nw", pady=3)
        self._sp_t = tk.Text(f, width=38, height=3, **eo)
        self._sp_t.insert("1.0", "\n".join(f"{k}={v}" for k,v in self.d.get("special_scoring",{}).items()))
        self._sp_t.grid(row=r, column=1, sticky="w", pady=3)
        r += 1
        tk.Label(f, text=L.t("band_pts"), **lo).grid(row=r, column=0, sticky="nw", pady=3)
        self._bp_t = tk.Text(f, width=38, height=3, **eo)
        self._bp_t.insert("1.0", "\n".join(f"{k}={v}" for k,v in self.d.get("band_points",{}).items()))
        self._bp_t.grid(row=r, column=1, sticky="w", pady=3)
        r += 1
        tk.Label(f, text=L.t("county_list"), **lo).grid(row=r, column=0, sticky="w", pady=3)
        self._cl_e = tk.Entry(f, width=50, **eo)
        self._cl_e.insert(0, ",".join(self.d.get("county_list",[])))
        self._cl_e.grid(row=r, column=1, sticky="w", pady=3)
        r += 1
        bf2 = tk.Frame(f, bg=TH["bg"])
        bf2.grid(row=r, column=0, columnspan=2, pady=18)
    ...
    def _save(self):
        if self.new:
            ...
            if not cid:
                messagebox.showerror(L.t("error"), "ID invalid!")
                return
            if cid in self.all_c:
                messagebox.showerror(L.t("error"), L.t("c_exists"))
                return
            self.cid = cid
        ...
        cats = [c.strip() for c in self._cats_t.get("1.0","end").splitlines() if c.strip()]
        self.d["categories"] = cats or ["Individual"]
        ...
        cl = self._cl_e.get().strip()
        self.d["county_list"] = [c.strip().upper() for c in cl.split(",") if c.strip()] if cl else []
...
class ContestMgr(tk.Toplevel):
    ...
    def _fill(self):
        ...
        for cid,cd in self.c.items():
            ...
            self.tree.insert("","end",iid=cid, values=(cid,nm,cd.get("contest_type","?"),cd.get("scoring_mode","none"),cd.get("multiplier_type","none"),cd.get("min_qso",0)))
    ...
    def _edit(self):
        cid = self._sel()
        if not cid: messagebox.showwarning("",L.t("no_sel")); return
    ...
    def _del(self):
        cid = self._sel()
        if not cid: return
        if self.c.get(cid,{}).get("is_default"): messagebox.showwarning("",L.t("c_default")); return
        if messagebox.askyesno(L.t("confirm_del"),L.t("del_c_conf").format(cid)):
        ...
        if fp:
            try:
                ...
                if isinstance(imp,dict):
                    added = sum(1 for cid,cd in imp.items() if cid not in self.c and not self.c.update({cid:cd}))
                    self._fill()
                    messagebox.showinfo("OK",f"Imported: {added}")
            except Exception as e: messagebox.showerror(L.t("error"),str(e))
...
class SearchDialog(tk.Toplevel):
    def __init__(self, parent, log_data):
        ...
        e = tk.Entry(self, textvariable=self._sv, width=40, **eo)
        e.pack(padx=10, pady=4, anchor="w")
        e.bind("<KeyRelease>", self._search)
        e.focus()
        self._lbl = tk.Label(self, text="", bg=TH["bg"], fg=TH["fg"], font=("Consolas",9))
        ...
        self.tree.pack(side="left", fill="both", expand=True)
        sb.pack(side="right", fill="y")
        tk.Button(self, text=L.t("close"), command=self.destroy, bg=TH["btn_bg"], fg="white").pack(pady=8)
        center_dialog(self, parent)

    def _search(self, e=None):
        q_str = self._sv.get().upper().strip()
        for i in self.tree.get_children(): self.tree.delete(i)
        if not q_str: self._lbl.config(text=""); return
        results = [(len(self.log_data)-i, q) for i,q in enumerate(self.log_data) if q_str in q.get("c","").upper() or q_str in q.get("n","").upper()]
        self._lbl.config(text=f"{L.t('results')}: {len(results)}")
        for nr,q in results:
            self.tree.insert("","end", values=(nr,q.get("c"),q.get("b"),q.get("m"),q.get("d"),q.get("n")))


class TimerDialog(tk.Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.title(L.t("timer_t")); self.geometry("300x220"); self.configure(bg=TH["bg"]); self.transient(parent)
        self._running = False; self._end_time = None; self._duration = 0; self._elapsed_start = None; self._elapsed_secs = 0
        lo = {"bg":TH["bg"],"fg":TH["fg"],"font":("Consolas",11)}
        eo = {"bg":TH["entry_bg"],"fg":TH["fg"],"font":("Consolas",11),"justify":"center","insertbackground":TH["fg"]}
        tk.Label(self, text=L.t("dur_h"), **lo).pack(pady=(15,0))
        self._dur_e = tk.Entry(self, width=10, **eo); self._dur_e.insert(0,"4"); self._dur_e.pack(pady=4)
...
class StatsWindow(tk.Toplevel):
    def __init__(self, parent, log_data, rules, cfg):
        super().__init__(parent)
        self.title(L.t("stats")); self.geometry("560x520"); self.configure(bg=TH["bg"]); self.transient(parent)
        center_dialog(self, parent)
        ...
        txt.tag_configure("ok", foreground=TH["ok"]); txt.tag_configure("err", foreground=TH["err"]); txt.tag_configure("warn", foreground=TH["warn"])
        ...
        if log_data:
            try:
                ...
                if len(dts)>=2:
                    ...
                    w(f"First: {dts[0].strftime('%Y-%m-%d %H:%M')}\nLast:  {dts[-1].strftime('%Y-%m-%d %H:%M')}\nDuration: {span_h:.1f}h  Rate: {len(log_data)/span_h:.1f} QSO/h\n")
            except: pass
        w("\n─── Benzi ───\n","h")
...
class Cab2ConfigDialog(tk.Toplevel):
    ...
    def __init__(self, parent, cfg):
        ...
        saved_sent = cfg.get("cab2_exch_sent", "none")
        default_sent = self._sent_values[-1]  # "Nimic (--)" default
        for display, key in self._sent_labels.items():
            if key == saved_sent:
                default_sent = display
                break

        self._sent_v = tk.StringVar(value=default_sent)
        ttk.Combobox(self, textvariable=self._sent_v, values=self._sent_values,
                     state="readonly", width=30, font=("Consolas", 11)).pack(padx=15, pady=4)
        ...
        saved_rcvd = cfg.get("cab2_exch_rcvd", "log")
        default_rcvd = self._rcvd_values[0]
        for display, key in self._rcvd_labels.items():
            if key == saved_rcvd:
                default_rcvd = display
                break

        self._rcvd_v = tk.StringVar(value=default_rcvd)
        ttk.Combobox(self, textvariable=self._rcvd_v, values=self._rcvd_values,
                     state="readonly", width=30, font=("Consolas", 11)).pack(padx=15, pady=4)

        # ── Buttons ──
        bf = tk.Frame(self, bg=TH["bg"])
        bf.pack(pady=18)
        tk.Button(bf, text=L.t("cab2_export"), command=self._ok,
                  bg=TH["ok"], fg="white", font=("Consolas", 12, "bold"), width=14).pack(side="left", padx=8)
        tk.Button(bf, text=L.t("cancel"), command=self.destroy,
                  bg=TH["btn_bg"], fg="white", font=("Consolas", 12), width=10).pack(side="left", padx=8)
        center_dialog(self, parent)

    def _ok(self):
        sent_key = self._sent_labels.get(self._sent_v.get(), "none")
        rcvd_key = self._rcvd_labels.get(self._rcvd_v.get(), "log")
        self.result = {"sent": sent_key, "rcvd": rcvd_key}
        self.destroy()
...
class App(tk.Tk):
    ...
    def _build_menu(self):
        ...
        for cid,cd in self.contests.items():
            nm = cd.get("name_"+L.g(), cd.get("name_ro",cid))
            cm.add_command(label=f"⚡ {nm}", command=lambda c=cid: self._switch_contest(c))
        tm = tk.Menu(mb, tearoff=0); mb.add_cascade(label=L.t("tools"), menu=tm)
        tm.add_command(label=L.t("search"), command=self._search_dlg)
        tm.add_command(label=L.t("timer"), command=self._timer_dlg); tm.add_separator()
        ...
        tm.add_command(label=L.t("verify"), command=self._verify_hash); tm.add_separator()
    ...
    def _build_ui(self):
        self._build_hdr(); self._build_inp(); self._build_flt(); self._build_tree(); self._build_btns()
    ...
    def _build_hdr(self):
        ...
        lc = ttk.Combobox(rf, textvariable=self.lang_v, values=["ro","en"], state="readonly", width=4); lc.pack(side="right", padx=3)
        ...
        self.ccb.pack(side="right", padx=3); self.ccb.bind("<<ComboboxSelected>>", self._on_cchange)
        self._upd_info()
    ...
    def _build_inp(self):
        ...
        self.ent["freq"].pack(); self.ent["freq"].bind("<FocusOut>", self._on_freq_out)
        ...
        self.ent["band"] = ttk.Combobox(bf2, values=ab, state="readonly", width=6, font=self.fn)
        self.ent["band"].set(ab[0] if ab else "40m"); self.ent["band"].pack()
        ...
        self.ent["mode"] = ttk.Combobox(mf2, values=am, state="readonly", width=6, font=self.fn)
        self.ent["mode"].set(am[0] if am else "SSB"); self.ent["mode"].pack()
        ...
        for k,lb in [("rst_s",L.t("rst_s")),("rst_r",L.t("rst_r"))]:
            ...
            e.insert(0, drst); e.pack(); self.ent[k] = e
    ...
    def _build_flt(self):
        ...
        ttk.Combobox(ff, textvariable=self.fb_v, values=[L.t("all")]+self._cc().get("allowed_bands",BANDS_ALL), state="readonly", width=7).pack(side="left", padx=3)
        ...
        ttk.Combobox(ff, textvariable=self.fm_v, values=[L.t("all")]+self._cc().get("allowed_modes",MODES_ALL), state="readonly", width=7).pack(side="left", padx=3)
        # bind filter changes
        for w in ff.winfo_children():
            if isinstance(w, ttk.Combobox): w.bind("<<ComboboxSelected>>", lambda e: self._refresh())
    ...
    def _build_btns(self):
        ...
        for txt,cmd,col in [
            (L.t("settings"),self._settings,TH["warn"]), (L.t("contests"),self._mgr,"#E91E63"),
            (L.t("stats"),self._stats,"#3F51B5"), (L.t("validate"),self._validate,TH["ok"]),
            (L.t("export"),self._export_dlg,"#9C27B0"), (L.t("import_log"),self._import_menu,"#FF5722"),
            (L.t("undo"),self._undo,"#795548"), (L.t("backup"),self._bak,"#607D8B"),
            (L.t("search"),self._search_dlg,"#00796B"), (L.t("timer"),self._timer_dlg,"#004D40"),
        ]:
    ...
    def _resolve_exchange_sent(self, q, mode="none"):
        """Resolve exchange sent value based on mode."""
        if mode == "county":
    ...
    def _resolve_exchange_rcvd(self, q, mode="log"):
        """Resolve exchange received value based on mode."""
        if mode == "log":
            sr = q.get("sr", "").strip()
            note = q.get("n", "").strip()
    ...
    def _add_qso(self):
        try: self._do_add_qso()
            ...
            import traceback
            messagebox.showerror(L.t("error"), f"Error:\n{e}\n{traceback.format_exc()[-300:]}")
    ...
    def _do_add_qso(self):
        ...
        if self.edit_idx is not None:
            ...
            self.log.insert(0,q); self.undo_stack.append(("add",0,q)); self.serial += 1
        self._clr(); self._refresh(); DM.save_log(self._cid(),self.log)

    def _clr(self):
        for k in ["call","note","freq"]: self.ent[k].delete(0,"end")
        if "ss" in self.ent: self.ent["ss"].delete(0,"end"); self.ent["ss"].insert(0,str(self.serial))
        if "sr" in self.ent: self.ent["sr"].delete(0,"end")
        if self.wb_lbl: self.wb_lbl.config(text="")
        self.ent["call"].focus()
    ...
    def _edit_sel(self):
        ...
        for k in ["rst_s","rst_r","note"]:
            fk = {"rst_s":"s","rst_r":"r","note":"n"}[k]
    ...
    def _on_lang(self, e):
        L.s(self.lang_v.get()); self.cfg["lang"] = self.lang_v.get(); DM.save("config.json",self.cfg); self._rebuild()
    ...
    def _about(self):
        ...
        tk.Label(d, text="📻 YO Log PRO v16.4", bg=TH["bg"], fg=TH["accent"], font=("Consolas",16,"bold")).pack(pady=12)
    ...
    def _settings(self):
        ...
        tk.Button(d, text=L.t("save"), command=save, bg=TH["accent"], fg="white", width=12).pack(pady=12)
        center_dialog(d, self)
    ...
    def _import_menu(self):
        ...
        for txt,cmd in [("ADIF (.adi/.adif)",lambda:[d.destroy(),self._import_adif()]),
    ...
    def _export_dlg(self):
        ...
        for txt,cmd in [("Cabrillo 3.0 (.log)",lambda:self._exp_cab(d)),
                        (L.t("exp_cab2"),lambda:self._exp_cab2(d)),
                        ("ADIF 3.1 (.adi)",lambda:self._exp_adif(d)),
                        ("CSV (.csv)",lambda:self._exp_csv(d)),
                        (L.t("exp_edi"),lambda:self._exp_edi(d)),
    ...
    def _exp_cab(self, parent=None):
        ...
        try:
            ...
            lines = ["START-OF-LOG: 3.0",f"CONTEST: {nm}",f"CALLSIGN: {my}",
                     f"GRID-LOCATOR: {self.cfg.get('loc','')}","CATEGORY-OPERATOR: SINGLE-OP",
                     "CATEGORY-BAND: ALL",f"CATEGORY-POWER: {cat_power}","CATEGORY-MODE: MIXED",
                     f"NAME: {self.cfg.get('op_name','')}",f"ADDRESS: {self.cfg.get('addr','')}",
                     "SOAPBOX: Logged with YO Log PRO v16.4",f"SOAPBOX: {self.cfg.get('soapbox','73 GL')}",
            ...
            content = "\n".join(lines)
            def do_save(text):
                fn = f"cab3_{self._cid()}_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}.log"
                fp = filedialog.asksaveasfilename(defaultextension=".log",filetypes=[("Cabrillo","*.log")],initialfile=fn)
                if fp:
    ...
    def _exp_cab2(self, parent=None):
        """Export Cabrillo 2.0 — with configurable exchange dialog."""
        if not self._check_before_export():
            return

        # ── Show config dialog to choose exchange ──
        cfg_dlg = Cab2ConfigDialog(self, self.cfg)
        self.wait_window(cfg_dlg)
        if not cfg_dlg.result:
            return  # cancelled

        exch_sent_mode = cfg_dlg.result["sent"]   # "county" / "grid" / "serial" / "none"
        exch_rcvd_mode = cfg_dlg.result["rcvd"]   # "log" / "none"

        # Save choice for next time
        self.cfg["cab2_exch_sent"] = exch_sent_mode
        self.cfg["cab2_exch_rcvd"] = exch_rcvd_mode
        ...
        try:
            ...
            if cat_val:
                ...
                if m_cat:
                    cat_num = str(ord(m_cat.group(1).upper()) - ord('A') + 1)
                else:
            ...
            def do_save(text):
                fn = f"cab2_{self._cid()}_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}.log"
    ...
    def _exp_adif(self, parent=None):
        ...
        try:
            ...
            lines = ["<ADIF_VER:5>3.1.0","<PROGRAMID:14>YO_Log_PRO_v16","<PROGRAMVERSION:5>16.4",f"<MY_GRIDSQUARE:{len(my_loc)}>{my_loc}","<EOH>"]
            for q in self.log:
            ...
            fp = filedialog.asksaveasfilename(defaultextension=".adi",filetypes=[("ADIF","*.adi")],initialfile=fn)
            if fp:
    ...
    def _exp_edi(self, parent=None):
        ...
        try:
            ...
            lines = ["[REG1TEST;1]",f"TName={nm}",f"TDate={now.strftime('%y%m%d')};{now.strftime('%y%m%d')}",f"PCall={my}",f"PWWLo={my_loc}","PExch=",f"PAdr1={self.cfg.get('addr','')}","PBand=144","PSect=","[Remarks]","Logged with YO Log PRO v16.4","[QSORecords]"]
            for q in self.log:
            ...
            fp = filedialog.asksaveasfilename(defaultextension=".edi",filetypes=[("EDI","*.edi")],initialfile=fn)
            if fp:
                with open(fp,"w",encoding="utf-8") as f: f.write("\n".join(lines))
                messagebox.showinfo(L.t("exp_ok"),f"→ {os.path.basename(fp)}")
            if parent: parent.destroy()
    ...
    def _exp_print(self, parent=None):
        ...
        try:
            ...
            lines = [f"{'='*90}",f"YO Log PRO v16.4 — {my} — {nm}",f"Generated: {datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M')} UTC",f"{'='*90}",
            ...
            lines.append(f"{'='*90}"); qp,mc,tot = Score.total(self.log,cc,self.cfg)
            ...
            fp = filedialog.asksaveasfilename(defaultextension=".txt",filetypes=[("Text","*.txt")],initialfile=fn)
            if fp:
                with open(fp,"w",encoding="utf-8") as f: f.write("\n".join(lines))
                messagebox.showinfo(L.t("exp_ok"),f"→ {os.path.basename(fp)}")
            if parent: parent.destroy()
