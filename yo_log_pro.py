...
class App(Tk):
    def __init__(self):
        ...
        fs = self.app_config.get("font_size", 11)
        self.font_size = int(fs)
        
        self.theme = {
            "bg": "#1e1e1e", "fg": "white", "accent": "#007acc", 
            "entry_bg": "#333333", "header": "#2d2d2d"
        }
    ...
    def _build_ui(self):
        ...
        curr_mode_key = self.app_config.get("active_contest", "simple")
        self.cb_mode.set(CONTEST_TYPES.get(curr_mode_key, "Log Simplu"))
        self.cb_mode.pack(side="left", padx=5)
        self.cb_mode.bind("<<ComboboxSelected>>", self._on_mode_change)

        info_text = str(self.app_config["callsign"]) + " | " + str(self.app_config["locator"])
        self.info_lbl = Label(header, text=info_text, fg="#81c784", bg=self.theme["header"], font=self.bold_font)
    ...
    def _draw_form(self):
        ...
        fields = [("Callsign", "call", 12), ("Band", "band", 8), ("Mode", "mode", 8), 
                  ("RST S", "rst_s", 5), ("RST R", "rst_r", 5), ("Note/Nume", "note", 20)]
        ...
        self.btn_log = Button(self.form_frame, text="LOG QSO", command=self._log_qso, 
                              bg=self.theme["accent"], fg="white", font=self.bold_font, padx=20)
    ...
    def _log_qso(self):
        ...
        qso_data = {
            ...
            "rst_s": self.entries["rst_s"].get(),
            "rst_r": self.entries["rst_r"].get(),
            "note": self.entries["note"].get(),
            "contest": self.cb_mode.get()
        }

        if self._editing_index is not None:
            self.log[self._editing_index] = qso_data
            self._editing_index = None
            self.btn_log.config(text="LOG QSO", bg=self.theme["accent"])
        else:
            self.log.insert(0, qso_data)
    ...
    def _on_tree_double_click(self, event):
        ...
        self.entries["mode"].set(qso["mode"])
        self.entries["rst_s"].delete(0, "end")
        self.entries["rst_s"].insert(0, qso["rst_s"])
    ...
    def _delete_qso(self):
        ...
        if not sel: return
        if messagebox.askyesno("Confirmare", "Stergeti QSO selectat?"):
            items = list(sel)
    ...
    def _open_settings(self):
        ...
        fields = [("Callsign:", "callsign"), ("Locator:", "locator"), 
                  ("Judet:", "judet"), ("Operator:", "operator")]
        set_entries = {}
        for label, key in fields:
            f = Frame(dlg, bg=self.theme["bg"], pady=5)
            f.pack(fill="x", padx=20)
            Label(f, text=label, fg="white", bg=self.theme["bg"]).pack(side="left")
            e = Entry(f, width=15)
            e.insert(0, self.app_config.get(key, ""))
            e.pack(side="right")
            set_entries[key] = e
        ...
        def save_settings():
            for key in set_entries:
                self.app_config[key] = set_entries[key].get().upper()
            self.app_config["category"] = cb_cat.get()
            self.app_config["font_size"] = int(sp_font.get())
            safe_save_json(FILES["config"], self.app_config)
            messagebox.showinfo("OK", "Setari salvate! Reporniti programul.")
            it = str(self.app_config["callsign"]) + " | " + str(self.app_config["locator"])
            self.info_lbl.config(text=it)
            dlg.destroy()

        Button(dlg, text="SALVEAZA", command=save_settings, bg=self.theme["accent"], fg="white", pady=10).pack(fill="x", padx=20, pady=20)
...
"
        for b in sorted(bands.keys()):
            stats_msg += str(b) + ": " + str(bands[b]) + " QSO
"
