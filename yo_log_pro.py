...
class DXCC:
    DB = {
        ...
        "DC": "Germany", "DD": "Germany", "DF": "Germany", "DG": "Germany", "DH": "Germany",
        "DM": "Germany",
        ...
        "PY": "Brazil", "PP": "Brazil", "PR": "Brazil", "PS": "Brazil", "PT": "Brazil",
        "PU": "Brazil",
...
DEFAULT_CONTESTS = {
    ...
    "yo-vhf": {
        "name_ro": "Contest VHF/UHF", "name_en": "VHF/UHF Contest", "contest_type": "VHF",
    ...
    "field-day": {
        "name_ro": "Ziua Câmpului", "name_en": "Field Day", "contest_type": "Field Day",
...
DEFAULT_CFG = {
    ...
    "sounds": True, "op_name": "", "power": "100", "win_geo": "",
}
...
class Importer:
    ...
    def parse_csv(text):
        ...
        for row in reader:
            ...
            q["d"] = row.get("Date") or row.get("DATE") or row.get("date") or \
...
class ContestEditor(tk.Toplevel):
    def __init__(self, parent, cid=None, cdata=None, all_c=None):
        ...
        title = L.t("edit_c") if not self.new else L.t("add_c")
        self.title(title)
    ...
    def _build(self):
        ...
        if self.new:
            ...
            self._e["id"].grid(row=r, column=1, sticky="w", **p)
            r += 1
        else:
            tk.Label(f, text=f"ID: {self.cid}", **lo).grid(row=r, column=0, columnspan=2, sticky="w",
                                                            **p)
            r += 1
        ...
        for k, lb in [("name_ro", L.t("c_name") + " (RO)"), ("name_en", L.t("c_name") + " (EN)")]:
            ...
            e.grid(row=r, column=1, sticky="w", **p)
            self._e[k] = e
            r += 1

        tk.Label(f, text=L.t("c_type"), **lo).grid(row=r, column=0, sticky="w", **p)
        self._tv = tk.StringVar(value=self.d.get("contest_type", "Simplu"))
        ttk.Combobox(f, textvariable=self._tv, values=CONTEST_TYPES, state="readonly", width=18).grid(
        ...
        ttk.Combobox(f, textvariable=self._sv, values=SCORING_MODES, state="readonly", width=18).grid(
            row=r, column=1, sticky="w", **p)
        r += 1
        ...
        ttk.Combobox(f, textvariable=self._mv, values=["none", "county", "dxcc", "band", "grid"],
                     state="readonly", width=18).grid(row=r, column=1, sticky="w", **p)
        r += 1
        ...
        for i, b in enumerate(BANDS_ALL):
            ...
            tk.Checkbutton(bf, text=b, variable=v, bg=TH["bg"], fg=TH["fg"],
                           selectcolor=TH["entry_bg"], activebackground=TH["bg"]).grid(
                row=i // 7, column=i % 7, sticky="w", padx=1)
            self._bv[b] = v
        r += 1

        tk.Label(f, text=L.t("a_modes"), **lo).grid(row=r, column=0, sticky="nw", **p)
        mf = tk.Frame(f, bg=TH["bg"])
        mf.grid(row=r, column=1, sticky="w", **p)
        self._modv = {}
        ...
        for i, m in enumerate(MODES_ALL):
            ...
            tk.Checkbutton(mf, text=m, variable=v, bg=TH["bg"], fg=TH["fg"],
                           selectcolor=TH["entry_bg"], activebackground=TH["bg"]).grid(
                row=i // 6, column=i % 6, sticky="w", padx=1)
            self._modv[m] = v
        r += 1
        ...
        tk.Checkbutton(f, text=L.t("use_serial"), variable=self._serv, bg=TH["bg"], fg=TH["fg"],
                       selectcolor=TH["entry_bg"], font=("Consolas", 11)).grid(
        ...
        self._cl = tk.Entry(f, width=50, **eo)
        self._cl.insert(0, ",".join(self.d.get("county_list", [])))
        self._cl.grid(row=r, column=1, sticky="w", **p)
        r += 1
        ...
        for k, lb, h, src in [
            ...
            t = tk.Text(f, width=40, height=h, **eo)
            t.insert("1.0", src)
            t.grid(row=r, column=1, sticky="w", **p)
            self._e[k] = t
            r += 1

        bf2 = tk.Frame(f, bg=TH["bg"])
        bf2.grid(row=r, column=0, columnspan=2, pady=12)
    ...
    def _save(self):
        if self.new:
            ...
            if not cid:
                messagebox.showerror(L.t("error"), "ID!")
                return
...
class ContestMgr(tk.Toplevel):
    ...
    def _build(self):
        ...
        for c, h, w in zip(cols,
                           ["ID", L.t("c_name"), L.t("c_type"), L.t("sc_mode"), L.t("cats"),
                            L.t("min_qso")],
                           [90, 170, 85, 85, 200, 45]):
    ...
    def _fill(self):
        ...
        for cid, cd in self.c.items():
            ...
            if len(cd.get("categories", [])) > 3:
                cats += "..."
            self.tree.insert("", "end", iid=cid, values=(
                cid, nm, cd.get("contest_type", "?"), cd.get("scoring_mode", "none"), cats,
                cd.get("min_qso", 0)))

    def _sel(self):
        s = self.tree.selection()
        if not s:
            messagebox.showwarning(L.t("error"), L.t("no_sel"))
            return None
        return s[0]
    ...
    def _del(self):
        ...
        if self.c.get(cid, {}).get("is_default"):
            messagebox.showwarning(L.t("error"), L.t("c_default"))
            return
            ...
            if isinstance(imp, dict):
                ...
                for cid, cd in imp.items():
                    if isinstance(cd, dict):
                        ...
                        n += 1
                self._fill()
                messagebox.showinfo("OK", f"Importate: {n}")
        except Exception as e:
            messagebox.showerror(L.t("error"), str(e))
...
class App(tk.Tk):
    ...
    def _build_hdr(self):
        ...
        self.st_lbl = tk.Label(lf, text=L.t("online"), bg=TH["header_bg"], fg=TH["led_on"],
                               font=self.fn)
        ...
        self.rate_lbl = tk.Label(rf, text="", bg=TH["header_bg"], fg=TH["ok"],
                                 font=("Consolas", 10))
        self.rate_lbl.pack(side="right", padx=8)

        self.lang_v = tk.StringVar(value=self.cfg.get("lang", "ro"))
        lc = ttk.Combobox(rf, textvariable=self.lang_v, values=["ro", "en"], state="readonly",
                          width=4)
    ...
    def _build_inp(self):
        ...
        tk.Label(r2, text=L.t("date_l"), bg=TH["bg"], fg=TH["fg"], font=self.fn).pack(side="left",
                                                                                        padx=3)
        ...
        tk.Label(r2, text=L.t("time_l"), bg=TH["bg"], fg=TH["fg"], font=self.fn).pack(side="left",
                                                                                        padx=3)
        ...
        tk.Label(r2, text=L.t("category"), bg=TH["bg"], fg=TH["fg"], font=self.fn).pack(
            side="left", padx=(12, 3))
        ...
        ttk.Combobox(r2, textvariable=self.cat_v, values=cats, state="readonly", width=20).pack(
            side="left", padx=2)

        if cc.get("use_county"):
            tk.Label(r2, text=L.t("county"), bg=TH["bg"], fg=TH["fg"], font=self.fn).pack(
                side="left", padx=(8, 3))
    ...
    def _build_flt(self):
        ...
        tk.Label(ff, text=L.t("f_band"), bg=TH["bg"], fg=TH["fg"],
                 font=("Consolas", 10)).pack(side="left")
        ab = [L.t("all")] + self._cc().get("allowed_bands", BANDS_ALL)
        self.fb_v = tk.StringVar(value=L.t("all"))
        fb_combo = ttk.Combobox(ff, textvariable=self.fb_v, values=ab, state="readonly", width=7)
        fb_combo.pack(side="left", padx=3)
        fb_combo.bind("<<ComboboxSelected>>", lambda e: self._refresh())
        tk.Label(ff, text=L.t("f_mode"), bg=TH["bg"], fg=TH["fg"],
        ...
        fm_combo = ttk.Combobox(ff, textvariable=self.fm_v, values=am2, state="readonly", width=7)
        fm_combo.pack(side="left", padx=3)
        fm_combo.bind("<<ComboboxSelected>>", lambda e: self._refresh())
        self.sc_lbl = tk.Label(ff, text="", bg=TH["bg"], fg=TH["gold"],
    ...
    def _build_tree(self):
        ...
        hdrs = [L.t("nr"), L.t("call"), L.t("freq"), L.t("band"), L.t("mode"), L.t("rst_s"),
                L.t("rst_r")]
    ...
    def _tick_clock(self):
        ...
        for q in self.log:
            try:
                qt = datetime.datetime.strptime(f"{q.get('d', '')} {q.get('t', '')}",
                                                "%Y-%m-%d %H:%M")
                if (now - qt).total_seconds() <= 3600:
                    cnt += 1
            except:
                pass
        if self.rate_lbl:
    ...
    def _undo(self):
        if not self.undo_stack:
            messagebox.showinfo(L.t("undo"), L.t("undo_empty"))
            return
    ...
    def _tog_man(self):
        ...
        self.ent["date"].config(state=st)
        self.ent["time"].config(state=st)
        lc = TH["led_off"] if m else TH["led_on"]
        if self.led_c and self.led:
    ...
    def _sort(self, col):
        ...
        try:
            ...
            items.sort(key=lambda t: t[0], reverse=self._sort_rev)
        for idx, (v, k) in enumerate(items):
    ...
    def _settings(self):
        ...
        for k, lb, v in [
            ...
            e = tk.Entry(d, width=38, **eo)
            e.insert(0, v)
            e.pack(pady=1, padx=15, fill="x")
            es[k] = e
        tk.Label(d, text=L.t("font_size"), bg=TH["bg"], fg=TH["fg"]).pack(anchor="w", padx=15)
        fs_e = tk.Entry(d, width=8, **eo)
        fs_e.insert(0, str(self.cfg.get("fs", 11)))
